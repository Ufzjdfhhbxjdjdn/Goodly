...
#    d = getpvk(rL[i[0]], sL[i[0]], zL[i[0]], rL[i[1]], sL[i[1]], zL[i[1]], int( i[2], 16) )
    ...
    for d in di:
        if ice.scalar_multiplication(d) == pub_point_Q:
            print(f'try Privatekey FOUND: {hex(d)}       Address: {address}')
...
# === Extra smart_tools analysis ===
try:
    import smart_tools
    print("\\n[smart_tools] Running additional intelligent analysis...")
    smart_tools.analyze_all(rL, sL, zL, [q.hex() for q in QL])

    for i in range(len(rL)):
        for j in range(i + 1, len(rL)):
            try:
                d_list = smart_tools.all_pvk_candidate(rL[i], sL[i], zL[i], rL[j], sL[j], zL[j], 0)
                for d in d_list:
                    if ice.scalar_multiplication(d) == QL[i]:
                        print(f"[smart_tools] Privatekey FOUND: {hex(d)}       Address: {address}")
            except:
                continue
